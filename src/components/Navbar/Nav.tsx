      ...
      <div className="flex items-center gap-3">
        {navItems.map((item) => (
          <NavLink
            ...
            to={item.path}
            className={({ isActive }) =>
              `px-4 py-2 rounded-full border transition-all duration-500 ease-in-out ${
                isActive
                  ? "bg-yellow-400 text-black border-yellow-400 shadow-[0_0_20px_rgba(250,204,21,0.5)]"
                  : "border-transparent text-white hover:bg-[#2e3306] hover:border-yellow-400"
              }`
            }
          >
            {item.name}
